...
describe('GameeVouchers', function () {
  ...
  beforeEach(async function () {
    const GameeVouchers = artifacts.require('artifacts_v1/contracts/solc-0.6/token/ERC1155/GameeVouchers.sol:GameeVouchers');
  ...
  describe('createCollection', function () {
    it('should revert if not called by the owner', async function () {
      await expectRevert(
        this.gameeVouchers.createCollection(Fungible.makeCollectionId(1), {
          from: payout,
        }),
    ...
    it('should emit a CollectionCreated event and set the creator', async function () {
      ...
      const receipt = await this.gameeVouchers.createCollection(collectionId, {
        from: deployer,
      });
  ...
  describe('safeBatchMint', function () {
    ...
    it('should fail when minting to a non-receiver contract', async function () {
      const ids = [Fungible.makeCollectionId(1)];
      const values = [1];
      await expectRevert(this.gameeVouchers.safeBatchMint(this.gameeVouchers.address, ids, values, '0x', {from: deployer}), 'revert');
    });
  });
});
